...
def telegram_bot(token_bot):
    print(f'Bot started: {authorized_users=}')
    count_secret = dict()
    ...
    def send_text(message):
        ...
        if not check_authorized_user(message):
            ...
            if key_count_secret in count_secret:
                if count_secret[key_count_secret] == 3:
                    bot.send_message(message.chat.id, wth_ans)
                    count_secret[key_count_secret] += 1
                elif count_secret[key_count_secret] > 3:
                    bot.send_message(message.chat.id, sec_hint)
                else:
                    count_secret[key_count_secret] += 1
                    bot.send_message(message.chat.id, try_again_ans)
            else:
                count_secret[key_count_secret] = 1
                bot.send_message(message.chat.id, try_again_ans)
        elif text_mes == sec_words_1 or text_mes == sec_words_2:
        ...
        elif text_mes == sec_words:
            bot.send_message(message.chat.id, answer_end)
